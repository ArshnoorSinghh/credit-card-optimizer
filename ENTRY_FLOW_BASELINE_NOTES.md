...
- `lib/profile-store.ts` — the held-card list lives in `cardIds` (key bumped
  `v1` → `v2`). On merge into `main` this unified with main's existing `cardIds`
  field, so held cards persist to Postgres via `/api/profile` for signed-in users
  rather than to sessionStorage alone.
...
- `/results` (deep-dive) reads `cardIds` from the store for its anchor. `/optimizer`
  no longer carries the list through by hand: main's `save(patch)` merges rather
  than replaces, so a spend edit can't wipe held cards.
