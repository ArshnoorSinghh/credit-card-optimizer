  ...
  for (const o of result.optionOutcomes) {
    ...
    if (o.merchantAssumption) {
      ...
      flags.push({
        ...
        // would describe the old, over-optimistic behaviour.
        message:
          rate.value === null && rate.range
            ? `${o.option.cardCategory}: pays only at ${o.merchantAssumption} — the share of your ${o.spendCategories.join("/")} spend going there is unknown, so this is scored as a 0-to-full range`
            : `${o.option.cardCategory}: confirmed at ${o.merchantAssumption} for this ${o.spendCategories.join("/")} spend`,
      });
    }
  }
